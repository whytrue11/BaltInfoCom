  ...
  public static void main(String[] args) {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(INPUT_FILE_PATH))) {
      ...
      while (line != null) {
        ...
        if (strings.size() != ELEMENTS_COUNT_IN_ROW) {
          line = reader.readLine();
          continue;
        }
    ...
    class Group {
      ...
      public Group(List<List<String>> rowsList, int startIndex, int endIndex) {
        group = new ArrayList<>(endIndex - startIndex);
    ...
    for (int column = 0; column < ELEMENTS_COUNT_IN_ROW; column++) {
      ...
      for (int i = 1; i < rowsList.size(); i++) {
        if (!startGroupString.isEmpty() && !rowsList.get(i).get(column).isEmpty() && startGroupString.equals(rowsList.get(i).get(column))) {
          endIndex = i;
        } else {
          ...
          startGroupString = rowsList.get(i).get(column);
          startIndex = i;
          endIndex = startIndex;
        }
      }
